...
Jeder dieser Stecker ist mit VCC (entweder +5V oder Motorspannung) und GND verbunden, sowie mit SDA und SCL (für die Kommunikation über das I2C Protokoll, mit Ausnahme Slot 5). Die mit S1 und S2 (Slot 1 und Slot 2) bezeichneten Leitungen sind je Port mit unterschiedlichen Pins am Arduino verbunden. Je nach Fähigkeiten dieser Pins sind die Ports unterschiedlich eingefärbt. 

Das Pinout-Diagramm zeigt nocheinmal die Belegung der Ports.
...
Die anderen Ports 5 bis 10 sind an +5V angeschlossen (mit Kurzschluss- und Überstromschutz für 3A)-
