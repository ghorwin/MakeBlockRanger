...
Nun beginnen wir mit der Programmierung. Auf dem Auriga-Board sind zahlreiche LEDs verbaut. In diesem Kapitel geht es darum, diese LEDs zum Leuchten zu bringen.
...
Dann gibt es noch eine grüne "On"-Status-LED (nicht programmierbar, hart verdrahtet), 2 rote LEDs direkt unter dem RGB-Ring (auch nicht programmierbar) und die blaue, rhythmisch blinkende LED oberhalb des Reset-Tasters (auch nicht programmierbar).
...
Als Zusatzmodul für das Auriga-Board gibt es einen RGB-Ring (Typ: WS2812).

.Ring aus 12 RGB-LEDs
image::RGBRing.gif[]
...
Alternativ (sehr detailliert, inkl. Messungen der tatsächlich akzeptierten Timings): https://cpldcpu.wordpress.com/2014/01/14/light_ws2812-library-v2-0-part-i-understanding-the-ws2812/[Understanding the WS2812]
...
Grundsätzlich hält die Klasse eine Datenstruktur für die RGB-Informationen jeder einzelnen LED. Die Konfiguration der LEDs kann nun durch Zugriffsfunktionen wie `setColorAt()` geändert werden. Dabei wird zunächst nur der interne Zustand des Klassenobjekts geändert. Die Ansteuerung des Controllers selbst und damit das Umschalten der LEDs erfolgt erst beim Aufruf von `show()`.

Die LED 0 (bzw. 1. LED) ist übrigens die LED auf "1 Uhr", wenn man sich das Bild oben anschaut. Die oberste LED ("12 Uhr") ist die LED 11 und LED 2 (bzw. 3. LED) zeigt in Richtung "Vorwärts" des Land Raider Modells.
