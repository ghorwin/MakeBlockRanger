...
Am Auriga-Board gibt es zwei Motorports. Jeder der DC-Motoren kann über einen PWM-Pin (analoge Spannung für die Geschwindigkeit) und zwei digitale Pins für die Drehrichtung angesteuert werden. Die Ansteuerung kann einmal direkt über die Arduino-Pins erfolgen oder unter Verwendung der Makeblock Bibliothek (in der die Details gekapselt sind).

[NOTE]
====
Grundsätzlich bewegen sich die Motoren nur, wenn die Spannungsversorgung über die Batterien erfolgt (die Motoren brauchen eine Spannung > 7V, was der USB-Port ja nicht liefern kann). Das ist auch ganz praktisch, da man so den Roboter angesteckt an den Computer programmieren kann, ohne Angst zu haben, dass er vom Tisch rollt.
====
...
void loop() {
  ...
  // alle 500 Millisekunden die Geschwindigkeit ausgeben
...
Dieses Programm hat viele Bestandteile, die man aber am Besten nach und nach erklärt.
...
[NOTE]
====
Nicht verwirren lassen: Im Pinout-Diagramm ist Pin 19 mit `INT1` beschriftet. Laut ATmega2560-Datenblatt ist Pin 19 aber `INT2` (Pin 18 ist tatsächlich `INT3`). Diese Hardware-Bezeichnungen haben ohnehin nichts mit den Interrupt-Nummern zu tun, die `attachInterrupt()` erwartet - deshalb sollte man die Nummer nie selbst hinschreiben, sondern immer `digitalPinToInterrupt()` verwenden.
====


Wie bei Encodern üblich, wird die Flanke des einen Pins überwacht und dann durch den (etwas versetzten) Pegel des zweiten Pins die Drehrichtung bestimmt. Dies geschieht in der Interrupt-Funktion:
...
Wenn man jetzt das Programm mal laufen lässt und die Geschwindigkeiten plottet, erhält man ein interessantes Bild:
...
Erneut getestet sieht die Geschwindigkeitsausgabe deutlich besser aus, d.h. die durch die Encoder erfasste Drehzahl ist auch bei kleineren Drehzahlen hinreichend proportional zur Sollgeschwindigkeit (in PWM), was man gut am freilaufenden Motor sehen kann.
...
Beim freilaufenden Motor wird die festgelegte Soll-Geschwindigkeit offenbar gut erreicht. Beim Motor mit Kette bleibt die tatsächlich erreichte Geschwindigkeit etwas hinter der Sollgeschwindigkeit zurück, was ja an der Bremswirkung liegt.
...
[TIP]
====
Wenn man den Land Raider mit aufgeladenen Batterien und Maximalgeschwindigkeit fahren lässt, so schafft er bei vollen Batterien auf gerader Fläche immerhin **0,56 m/s** bzw. **2 km/h**. Das ist zwar langsamer als die oben ausgerechnete Maximalgeschwindigkeit, reicht aber locker, um durch eine Legostadt zu düsen und spektakuläre Videos aus der Legomännelperspektive zu filmen.
====

[NOTE]
====
Als ich das Testprogramm gerade nochmal mit frisch aufgeladenen Akkus getestet habe, sind bei PWM 255 durchaus Drehzahlen bis 388 RPM angezeigt worden. Die Akkuspannung hat also einen erheblichen Einfluss darauf, was an Drehzahlen und Geschwindigkeiten so möglich ist.
====
...
## Motorleistung adaptiv regeln mittels der Encoder-Klasse
