...
Einige der Ein- und Ausgänge sind direkt mit Komponenten auf dem Auriga-Board verbunden. Andere sind mit externen Ports und Anschlüssen verbunden. Die Details dazu findet man auf dem link:../downloads/MeAuriga_Schaltplan.pdf[Auriga Schaltplan]. Der ist allerdings nicht leicht zu lesen und enthält auch viele Informationen, welche man bei Verwendung der MakeBlock-Programmierbibliothek gar nicht benötigt.
...
MePort_Sig mePort[17] = {
	...
	Port 11 {  NC,  A2 }   // Lichtsensor (befindet sich neben Port 2)
	Port 12 {  NC,  A3 }   // Lichtsensor (befindet neben Port 9)
	Port 13 {  NC,  A0 }   // Temperatursensor
	Port 14 {  NC,  A1 }   // Mikrofon/Lautstärkesensor
	Port 15 {  NC,  NC }   
	Port 16 {  NC,  NC }   
}
```
...
Als Beispiel: der Port 6 ist an die analogen Pins A10 und A15 des Mikrocontrollers angeschlossen.

[NOTE]
====
Der Kommentar in der Original-Header-Datei bezeichnet Port 11 als `LIGHT2` und Port 12 als `LIGHT1`. Die Nummerierung der beiden Lichtsensoren ist also nicht eindeutig. Im Kapitel <<chap:sensors>> werden sie deshalb über ihre Einbaulage unterschieden (der eine sitzt neben Port 2, der andere neben Port 9).
====
