...
Beim Blick in die Include-Datei `MeAuriga.h` fällt einem am Schluss der Datei folgender Quelltext auf:
...
Man kann mittels der Member-Funktion `MeBuzzer::tone(frequenz, dauer in ms)` einen Ton ausgeben. Diese Funktion erzeugt das Tonsignal per Software (d.h. in dieser Funktion wird der angegebene Buzzer-Pin sehr häufig an- und ausgeschaltet, wobei die _Frequenz_ dieses Umschaltens die Tonhöhe festlegt). Dabei blockiert die Funktion für die angegebene Dauer.
...
[WARNING]
====
Für die Verwendung des OnBoard-Buzzers sollte man die `MeBuzzer`-Klasse _nicht_ benutzen, sondern stattdessen wie im Beispiel oben via `tone()` die Frequenz einstellen. Das hat auch den Vorteil, dass man ohne Wartezeiten parallel zum Abspielen einer Melodie noch Sensoren auslesen und andere Dinge steuern kann.
====
